...
public class SistemaMonitoria {
	...
	/*
	  criarDiscussao(...) * -> Deveria estar Modelado nesta classe
	  editarDiscussão(...) * ->Deveria estar Modelado nesta classe
	  avaliarMonitor(...) * -> Os parametros modelados não são suficientes
	  adicionarMateria(...) OK
	  excluirMateria(...) OK
	  editarMateria(...) OK
	  adicionarAluno(...) OK
